...
browser.cookies.onChanged.addListener(({ cookie, cause }) => {
  ...
  console.debug(`${name}@${domain} updated (${cause})`);
...
const setLocalhostCookie = async ({ value, name }: Cookies.Cookie) => {
  try {
    ...
    console.debug(`${name}@localhost set`);
  } catch (error) {
    if (error instanceof Error) {
      console.error(`Error setting ${name}@localhost: ${error.name} - ${error.message}`);
    } else {
      console.error(`Error setting ${name}@localhost: ${error}`);
    }
  }
};
...
const removeLocalhostCookie = async (name: string) => {
  try {
    ...
    console.debug(`${name}@localhost removed`);
  } catch (error) {
    if (error instanceof Error) {
      console.error(`Error removing ${name}@localhost: ${error.name} - ${error.message}`);
    } else {
      console.error(`Error removing ${name}@localhost: ${error}`);
    }
  }
};
