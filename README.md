...
[![Install for Firefox](https://img.shields.io/badge/Firefox-Install%20Extension-FF7139?logo=firefox-browser&logoColor=white)](https://github.com/cskrov/nav-dev-sessions-extension/releases/latest/download/nav_dev_sessions_extension-firefox.xpi)
...
3. Add a mapping from the `dev` or `prod` domain to `localhost:<port>`.
4. Open your `localhost:<port>` application in another tab.
5. Enjoy seamless authentication on your `localhost` application!
6. Continue to use `dev` or `prod` in the first tab as normal.
   - This is recommended to keep the session cookies fresh. 
